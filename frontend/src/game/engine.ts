...
export class GameEngine {
  ...
  private tileAt(tx: number, ty: number): string {
    ...
    // Below the level grid must be OPEN, never a solid cap. A solid floor here
    // would silently catch a player who fell through a pit and leave them stuck
    // underneath the tiles with no death. Leaving it empty lets them keep
    // falling into the fall/death boundary in update(). Ceiling (ty < 0) and the
    // left/right edges stay solid so the player is still bounded on those sides.
    if (ty >= rows.length) return ".";
    if (ty < 0) return "#"; // solid ceiling cap
    const row = rows[ty];
