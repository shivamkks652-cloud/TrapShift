...
- UPDATE 2 (user screenshot): manifest merger fix WORKED (build reached compile stage) but new failure: `:capacitor-android:compileDebugJavaWithJavac FAILED — error: invalid source release: 21` = JAVA_HOME points to an older JDK; Capacitor 8 needs JDK 21. User is on Windows (D:\TrapShift\frontend, PowerShell). android-fix.mjs UPGRADED: auto-detects Android Studio bundled JBR (JDK 21) at common paths (C:\Program Files\Android\Android Studio\jbr etc.), passes JAVA_HOME to all gradle spawns, AND writes org.gradle.java.home into android/gradle.properties so Android Studio builds also use it. Syntax-checked + escaping verified.
